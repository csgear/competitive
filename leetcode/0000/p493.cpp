...
// BIT solution: process from right to left, query count of numbers > 2 * nums[i]
...
class Solution {
   public:
    int reversePairs(vector<int>& nums) {
        // leetcode style coordinate compression
        ...
        // insert x * 2 is very clever that we need to query for numbers > 2 * nums[i]
        int ret = 0;
        BIT bit(values.size());
